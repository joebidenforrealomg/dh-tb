...
const workingProxyList = document.getElementById('workingProxyList');
...
const appendListItem = (proxy, isControl = false) => {
  ...
  if (isControl) {
    ...
    checkProxy(proxy).then((isAccessible) => {
      if (isAccessible) {
        workingProxyList.appendChild(listItem);
      } else {
    ...
    checkProxy(proxy).then((isAccessible) => {
      ...
      listItem.classList.add(isAccessible ? 'yea' : 'nah');
...
const loadProxies = (filePath) => {
  workingProxiesElement.innerText = "Please wait...";
  fetch(filePath)
